...
export class TransactionService {
  ...
  async getBalanceHistory(
    ...
    try {
      ...
      const accountMap = new Map(accounts.map((acc) => [acc.id, { balance: acc.balance }]));
  ...
  async exportTransactionsToJSON(
    ...
    try {
      log.info({ userId, filters }, 'Starting JSON export');
      ...
      log.info({ userId, transactionCount: transactions.length }, 'JSON export completed');
